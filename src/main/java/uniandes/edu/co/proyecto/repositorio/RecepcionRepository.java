...
public interface RecepcionRepository extends JpaRepository<Recepcion, Integer> {
    ...
    @Query(value = "SELECT r.* FROM recepcion r INNER JOIN ordencompra ON r.ordencompra_id = ordencompra.id WHERE r.fecharecepcion >= SYSDATE - 30 AND r.bodega_id = :idB and ordencompra.sucursal_id = :idS", nativeQuery = true)
