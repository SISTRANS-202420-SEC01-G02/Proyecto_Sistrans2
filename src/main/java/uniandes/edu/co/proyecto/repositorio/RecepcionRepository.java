...
public interface RecepcionRepository extends JpaRepository<Recepcion, Integer> {
    ...
    @Query(value = "UPDATE ProductoBodega PB SET "+//
                    ...
                    "PB.costopromedio = ROUND(((PB.cantidadproducto * PB.costopromedio) + ((SELECT SELECT ProductoCompra.precioacordado FROM OrdenCompra INNER JOIN ProductoCompra ON OrdenCompra.id = ProductoCompra.ordencompra_id WHERE OrdenCompra.id = :ordencompra_id)*(SELECT ProductoCompra.cantidad FROM OrdenCompra INNER JOIN ProductoCompra ON OrdenCompra.id = ProductoCompra.ordencompra_id WHERE OrdenCompra.id = :ordencompra_id)))/(PB.cantidadproducto + (SELECT ProductoCompra.cantidad FROM OrdenCompra INNER JOIN ProductoCompra ON OrdenCompra.id = ProductoCompra.ordencompra_id WHERE OrdenCompra.id = :ordencompra_id))) "+//
